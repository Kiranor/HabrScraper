...
def writeToFile(name,text,path,format = '.txt'):
    ...
    with open(path + name + format, 'w', encoding='utf-8') as f:
        f.write(text)
...
writeToFile(BeautifulSoup(html,'lxml').title.text.split('/')[0], html, htmlPath,'.html')
...
for ref in soup.find('div', class_='dropdown-container dropdown-container_flows').find_all(class_='n-dropdown-menu__item-link n-dropdown-menu__item-link_flow'):
    text = ref.text
    link = ref.get('href')
...
for newUrl in links.values():
    ...
    writeToFile(BeautifulSoup(currentPage,'lxml').title.text.split('/')[0].split(' ')[-2], currentPage, htmlPath,'.html')
    try:
